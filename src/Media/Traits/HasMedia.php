...
trait HasMedia
{
     ...
     */
    public function attachMedia(Media|int|string|Collection $media, string $channel = 'default'): static
    {
        $mediaIds = $this->parseMediaIds($media);
        $params = collect($mediaIds)->mapWithKeys(
            fn($id) => [$id => ['channel' => $channel]]
        )->toArray();

        throw_unless($params, ModelNotFoundException::class);

        $this->media()->sync($params, false);
     ...
     * @return int|null
     */
    public function detachMedia(Media|int|string $media = null): ?int
    {
