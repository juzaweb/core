                    ...
                    <ul class="nav nav-tabs mb-0" id="myTab" role="tablist">
                        <li class="nav-item">
                            <a class="nav-link active" id="upload-tab" data-toggle="tab" href="#upload" role="tab">
                                <i class="fa fa-cloud-upload-alt"></i> {{ __('admin::translation.upload_media') }}
                            </a>
                        </li>
                        @if (config('media.upload_from_url'))
                            <li class="nav-item">
                                <a class="nav-link" id="import-tab" data-toggle="tab" href="#import" role="tab">
                                    <i class="fa fa-link"></i> {{ __('admin::translation.upload_from_url') }}
                                </a>
                            </li>
                        @endif
                    </ul>
                    <button type="button" class="close" data-dismiss="modal"
                        aria-label="{{ __('admin::translation.close') }}">
                        ...
                        <div class="upload-container p-2">
                            <form action="{{ route('media.upload', [$websiteId, 'public']) }}" role="form"
                                ...
                                <div class="dz-message text-center">
                                    ...
                                    <h4>{{ __('admin::browser.message-drop') }}</h4>
                                    <p class="text-muted">{{ __('admin::browser.message-choose') }}</p>
                                </div>
                            </form>
                        </div>
                    </div>
                    ...
                    <div class="tab-pane fade" id="import" role="tabpanel" aria-labelledby="import-tab">
                        <div class="upload-container p-4">
                            <form action="{{ route('media.import', [$websiteId, 'public']) }}" role="form"
                                ...
                                {{ Field::text(__('admin::translation.url'), 'url', ['required' => true]) }}

                                <div class="form-check">
                                    <input type="checkbox" name="download" class="form-check-input" value="1"
                                        id="download-checkbox" checked>
                                    <label class="form-check-label"
                                        for="download-checkbox">{{ __('admin::translation.download_to_server') }}</label>
                                </div>

                                <input type="hidden" name="working_dir" id='working_dir' value="{{ $folderId }}">

                                <button type="submit" class="btn btn-success mt-2">
                                    <i class="fa fa-cloud-upload"></i> {{ __('admin::translation.upload_file') }}
                                </button>
                            </form>
                        </div>
                    </div>
