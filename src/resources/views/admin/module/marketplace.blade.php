    ...
    <script type="text/javascript" nonce="{{ csp_script_nonce() }}">
        $(function() {
            ...
            function loadData() {
                ...
                $.ajax({
                    type: 'GET',
                    url: '{{ route('admin.modules.marketplace.get-data') }}',
                    dataType: 'json',
                    cache: false,
                    data: {
                        page: page,
                        ...
                        if (response.success) {
                            ...
                            if (page === 1) {
                                $('#marketplace-list').html(
                                    '<div class="col-md-12"><div class="alert alert-warning">' +
                                    '<i class="fa fa-exclamation-triangle"></i> ' +
                                    (response.message || '{{ __('core::translation.no_modules_found') }}') +
                                    '</div></div>'
                                );
                            }
                        }
